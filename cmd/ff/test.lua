...
test('ff sub', function()
  local f = io.open('.out/TEST', 'w+')
  ff{dir..'a.txt', r=true, pat='(a %d)1', sub='%1A', log=f}
  assertEq(
    expectSimple('.out/ff/a.txt', '    %i1: a %iA'),
    seekRead(f))

  f:close(); f = io.open('.out/TEST', 'w+')
  ff{dir..'a.txt', m=true, r=true, pat='(a %d)1', sub='%1A',
    log=f}
  assertEq(
    expectSimple('.out/ff/a.txt', '    %i1: a %iA'),
    seekRead(f))
  ...
  f:close(); f = io.open('.out/TEST', 'w+')
  ff{dir..'a.txt', r=true, pat='a %d1', log=f}
  assertEq('', seekRead(f))

  -- change it back
  ff{dir..'a.txt', m=true, r=true, pat='(a %d)A', sub='%11',
    log=f}
  assertEq(
    expectSimple('.out/ff/a.txt', '    %i1: a %i1'),
    seekRead(f))
